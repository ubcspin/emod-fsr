def calculate_keystroke_features_per_key(df, key='a1'):
    ...
    start_index = pd.Index(df[raw_key + '_press_start']).get_indexer_for([True])
    ...
    column_names = [key + '_keystroke_duration', key + '_keystroke_peak_count',
                    key + '_keystroke_max_peak', key + '_keystroke_var',
                    key + '_keystroke_mean', key + '_keystroke_auc',
                    key + '_keystroke_time_to_peak', key + '_keystroke_peak_to_release',
                   ]
    ...
    for s, e in zip(start_index, end_index):
        df.loc[s:e, key + '_keystroke_duration'] = df.timestamps.iloc[e] - df.timestamps.iloc[s]
        ...
        try:
            ...
            df.loc[s:e, key + '_keystroke_time_to_peak'] = timestamps[peak_max_arg] - df.timestamps.iloc[s]
            df.loc[s:e, key + '_keystroke_peak_to_release'] = df.timestamps.iloc[e] - timestamps[peak_max_arg]
            df.loc[s:e, key + '_keystroke_var'] = np.var(data)
            df.loc[s:e, key + '_keystroke_mean'] = np.mean(data)
            df.loc[s:e, key + '_keystroke_auc'] = auc_group(x=timestamps, y=data)
...
def calculate_keystroke_features(df, time_index='timedelta', time_interval='5s', columns=None):
    dfs = []
    if columns is None:
